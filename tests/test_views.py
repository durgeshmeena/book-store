...
def test_add_book_route(test_client):
    ...
    # try adding a book
    response = test_client.post('/add-book', data={
        "title": "New Book",
        "authors": "New Author",
        "isbn": "1234567890",
...
def test_get_member_details(test_client):
    ...
    member = Member(username="testuser", email="testuser@example.com", password="password", admin=False, active=True)
...
def test_update_member(test_client):
    # add member in test database
    member = Member(username="testuser", email="testuser@example.com", password="password", admin=False, active=False)
    member.set_password("password")
    member.save()
    # admin login
    with test_client.session_transaction() as session:
    ...
    response = test_client.post(f'/member/{member_id}', data={
        'name': 'My Name',
        'email': 'testuser@example.com',
        'balance': 100,
        'active': True
